...
import {
  pdrbData, inflasiData, pengangguranData, kemiskinanData,
} from "@/lib/data/bps";
import { biRateHistory, jisdorData, cadanganDevisaData } from "@/lib/data/bi";
import { neracaPerdagangan } from "@/lib/data/kemendag";
import { bungaPenjaminanRupiah, indeksKepercayaan } from "@/lib/data/lps";
import { asetPerbankan, nplData, pertumbuhanKredit } from "@/lib/data/ojk";
import { penerimaanNegara, belanjaNegara } from "@/lib/data/kemenkeu";
import { calculateCorrelation, type CorrelationResult } from "@/lib/correlation";
import ScatterPlot from "@/components/charts/scatter-plot";

interface Dataset { id: string; name: string; category: string; source: string; data: Array<{ period: string; value: number }> }

const datasets: Dataset[] = [
  { id: "pdrb", name: "PDB Indonesia", category: "Makroekonomi", source: "BPS", data: pdrbData },
  { id: "inflasi", name: "Inflasi (CPI)", category: "Makroekonomi", source: "BPS", data: inflasiData },
  { id: "pengangguran", name: "Pengangguran Terbuka", category: "Makroekonomi", source: "BPS", data: pengangguranData },
  { id: "kemiskinan", name: "Tingkat Kemiskinan", category: "Makroekonomi", source: "BPS", data: kemiskinanData },
  { id: "bi-rate", name: "BI-Rate", category: "Moneter", source: "Bank Indonesia", data: biRateHistory },
  { id: "jisdor", name: "Kurs JISDOR (USD/IDR)", category: "Moneter", source: "Bank Indonesia", data: jisdorData },
  { id: "cadangan-devisa", name: "Cadangan Devisa", category: "Moneter", source: "Bank Indonesia", data: cadanganDevisaData },
  { id: "neraca-perdagangan", name: "Neraca Perdagangan", category: "Perdagangan", source: "Kemendag", data: neracaPerdagangan },
  { id: "bunga-penjaminan", name: "Suku Bunga Penjaminan", category: "Perbankan", source: "LPS", data: bungaPenjaminanRupiah },
  { id: "indeks-kepercayaan", name: "Indeks Kepercayaan Konsumen", category: "Perbankan", source: "LPS", data: indeksKepercayaan },
  { id: "aset-perbankan", name: "Total Aset Perbankan", category: "Perbankan", source: "OJK", data: asetPerbankan },
  { id: "npl", name: "NPL (Kredit Bermasalah)", category: "Perbankan", source: "OJK", data: nplData },
  { id: "pertumbuhan-kredit", name: "Pertumbuhan Kredit", category: "Perbankan", source: "OJK", data: pertumbuhanKredit },
  { id: "penerimaan-negara", name: "Penerimaan Negara", category: "Fiskal", source: "Kemenkeu", data: penerimaanNegara },
  { id: "belanja-negara", name: "Belanja Negara", category: "Fiskal", source: "Kemenkeu", data: belanjaNegara },
];

const categories = [...new Set(datasets.map((d) => d.category))];

function DatasetSelector({ label, value, search, setSearch, showDropdown, setShowDropdown, filtered, onSelect, excludeId }: {
  label: string; value: string; search: string; setSearch: (s: string) => void; showDropdown: boolean; setShowDropdown: (b: boolean) => void; filtered: Dataset[]; onSelect: (id: string) => void; excludeId?: string;
}) {
...
export default function KorelasiPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <DatasetSelector label="Variabel X" value={selectedX} search={searchX} setSearch={setSearchX} showDropdown={showDropdownX} setShowDropdown={setShowDropdownX} filtered={filteredX} onSelect={setSelectedX} excludeId={selectedY} />
        <DatasetSelector label="Variabel Y" value={selectedY} search={searchY} setSearch={setSearchY} showDropdown={showDropdownY} setShowDropdown={setShowDropdownY} filtered={filteredY} onSelect={setSelectedY} excludeId={selectedX} />
      </motion.div>
