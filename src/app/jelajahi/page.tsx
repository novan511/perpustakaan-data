...
import { allCategories, searchDatasets, type DatasetItem } from "@/lib/data/categories";
...
export default function JelajahiPage() {
  ...
  const filteredCategories = useMemo(() => {
    if (!search && !selectedCategory) return allCategories;
    if (selectedCategory) {
    ...
    const results = searchDatasets(search);
    const catIds = new Set(results.map((d) => allCategories.find((c) => c.datasets.includes(d))?.id));
    return allCategories.filter((c) => catIds.has(c.id)).map((c) => ({
      ...c,
  ...
  const totalDatasets = allCategories.reduce((acc, c) => acc + c.datasets.length, 0);
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">
            {allCategories.length} kategori &middot; {totalDatasets} dataset &middot; Tanpa batas
          </p>
        </div>
      </div>
