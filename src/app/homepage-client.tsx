    ...
    <div className="bg-slate-900 dark:bg-black border-b border-slate-800 overflow-hidden">
      ...
      >
        {[...items, ...items, ...items].map((item, i) => (
          <div
            key={i}
            className="flex items-center gap-2 px-4 py-1.5 text-[10px] border-r border-slate-800"
          >
...
function FeaturedStory({ facts }: { facts: FunFact[] }) {
...
function FunFactCard({ fact, index }: { fact: FunFact; index: number }) {
  ...
  return (
    ...
    >
      <Link
        href="/tanya"
        className="group block bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 sm:p-5 hover:shadow-lg transition-all"
      >
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-8">
                ...
                <motion.div key={i} variants={fadeUp}>
                  <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 hover:shadow-lg transition-shadow">
                    <div className="flex items-center gap-2 mb-2">
                      <div
                        className={`w-8 h-8 rounded-lg bg-gradient-to-br ${kpi.color} flex items-center justify-center`}
                      >
        ...
        {(inflasiData.length > 0 || jisdorData.length > 0) && (
        ...
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">
              Alat Keputusan Keuangan
            </h3>
          </div>
          <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mb-4">
            Masukkan situasi kamu, dapatkan jawaban yang bisa langsung dipakai untuk
            mengambil keputusan.
          </p>
          <motion.div
            variants={stagger}
          ...
          >
            {tools.map((tool) => (
              <motion.div key={tool.href} variants={fadeUp}>
                <Link
                  href={tool.href}
                  className="group block bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 hover:shadow-lg transition-all"
                >
                  <div
                    className={`w-9 h-9 rounded-xl bg-gradient-to-br ${tool.color} flex items-center justify-center mb-2`}
                  >
                    <tool.icon className="w-4 h-4 text-white" />
                  </div>
                  <h4 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white group-hover:text-red-500 transition-colors">
                    {tool.title}
                  </h4>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Sumber Data", value: stats.sources, icon: Globe },
            { label: "Dataset", value: stats.datasets, icon: BarChart3 },
            { label: "Alat Keputusan", value: stats.tools, icon: Calculator },
            { label: "Pertanyaan", value: stats.questions, icon: Users },
          ].map((stat, i) => (
            <motion.div
              key={i}
