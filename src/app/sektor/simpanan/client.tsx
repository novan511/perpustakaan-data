...
export default function SimpananClient({ category }: { category: SupabaseCategory | null }) {
  const datasets = category?.datasets || [];
  const kpis = datasetsToKPIs(datasets);

  const bungaDataset = findDataset(datasets, "bunga-penjaminan", "bunga", "interest");
  const indeksDataset = findDataset(datasets, "indeks", "kepercayaan", "index");
  const distribusiDataset = findDataset(datasets, "distribusi", "distribution");

  const bungaData = datasetToDataPoints(bungaDataset);
  const indeksData = datasetToDataPoints(indeksDataset);
  const distribusiData = datasetToDataPoints(distribusiDataset);

  const bungaFormat = createFormatValue(bungaDataset?.unit || "%");
  const indeksFormat = createFormatValue(indeksDataset?.unit || "");

  const latestBunga = bungaData[bungaData.length - 1];
  const latestIndeks = indeksData[indeksData.length - 1];

  const prevBunga = bungaData.length >= 2 ? bungaData[bungaData.length - 2] : null;
  const prevIndeks = indeksData.length >= 2 ? indeksData[indeksData.length - 2] : null;

  const bungaChange = prevBunga ? Math.round(((latestBunga.value - prevBunga.value) / Math.abs(prevBunga.value)) * 10000) / 100 : 0;
  const indeksChange = prevIndeks ? Math.round(((latestIndeks.value - prevIndeks.value) / Math.abs(prevIndeks.value)) * 10000) / 100 : 0;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{category?.name || "LPS - Lembaga Penjamin Simpanan"}</h2>
          <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">
            Penjaminan simpanan &middot;{" "}
            <a href={bungaDataset?.source_url || "https://lps.go.id"} target="_blank" rel="noopener noreferrer" className="text-red-500 hover:underline">lps.go.id</a>
          </p>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
        <KPIInsight
          ...
          value={latestBunga ? bungaFormat(latestBunga.value) : "-"}
          change={bungaChange}
        ...
        <KPIInsight
          ...
          value={latestIndeks ? indeksFormat(latestIndeks.value) : "-"}
          change={indeksChange}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        {bungaData.length > 0 && (
          <ChartWrapper
            ...
            description={bungaDataset?.unit || "Batas atas bunga (%)"}
            data={bungaData}
            color="#ec4899"
            type={getChartTypeForDataset(bungaDataset?.slug || "")}
            sources={[category?.name || "LPS"]}
            formatValue={bungaFormat}
          />
        )}
        {indeksData.length > 0 && (
          <ChartWrapper
            title="Indeks Kepercayaan Konsumen"
            description={indeksDataset?.unit || ""}
            data={indeksData}
            color="#06b6d4"
            sources={[category?.name || "LPS"]}
            formatValue={indeksFormat}
          />
        )}
      </div>
      ...
      {distribusiData.length > 0 && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 sm:p-5">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={distribusiData} margin={{ top: 5, right: 5, left: -10, bottom: 5 }}>
              ...
              <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                {distribusiData.map((_, index) => (<Cell key={index} fill={COLORS[index % COLORS.length]} />))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        {bungaData.length > 0 && <DataTable title="Suku Bunga Penjaminan" data={bungaData} headers={["Periode", "Bunga (%)"]} sources={[category?.name || "LPS"]} />}
        {indeksData.length > 0 && <DataTable title="Indeks Kepercayaan" data={indeksData} headers={["Periode", "Indeks"]} sources={[category?.name || "LPS"]} />}
      </div>
    </div>
  );
}
