...
import { kemenkeuKPIs } from "@/lib/data";
import { penerimaanNegara, belanjaNegara, defisitData, komposisiAPBN, alokasiBelanja } from "@/lib/data/kemenkeu";
import { ArrowLeft } from "lucide-react";
import Link from "next/link";
import { PieChart, Pie, Tooltip, ResponsiveContainer } from "recharts";

export default function FiskalPage() {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Kementerian Keuangan</h2>
          <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">
            APBN & keuangan negara &middot;{" "}
            <a href="https://www.kemenkeu.go.id" target="_blank" rel="noopener noreferrer" className="text-red-500 hover:underline">kemenkeu.go.id</a>
          </p>
        </div>
      </motion.div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {kemenkeuKPIs.map((kpi, i) => <KPICard key={i} {...kpi} />)}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <ChartWrapper title="Penerimaan vs Belanja Negara" description="Rp T per tahun" data={penerimaanNegara} color="#3b82f6" secondData={belanjaNegara} secondColor="#10b981" secondLabel="Belanja" sources={["Kemenkeu"]} formatValue={(v) => `Rp ${v.toLocaleString("id-ID")} T`} />
        <ChartWrapper title="Defisit APBN" description="Rp T" data={defisitData} color="#ef4444" type="bar" sources={["Kemenkeu"]} formatValue={(v) => `Rp ${v.toLocaleString("id-ID")} T`} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 sm:p-5">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={komposisiAPBN} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={3} dataKey="value" label />
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 sm:p-5">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={alokasiBelanja} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={3} dataKey="value" label />
              <Tooltip contentStyle={{ backgroundColor: "white", border: "1px solid #e2e8f0", borderRadius: "12px", fontSize: "12px" }} />
            </PieChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <DataTable title="Penerimaan Negara" data={penerimaanNegara} headers={["Tahun", "Penerimaan (Rp T)"]} sources={["Kemenkeu"]} />
        <DataTable title="Belanja Negara" data={belanjaNegara} headers={["Tahun", "Belanja (Rp T)"]} sources={["Kemenkeu"]} />
      </div>
    </div>
  );
}
